...
public class JAvaMulakatSorulari {
...
– Cohesion Nedir?
Bir sınıfın oluşturulma amacını ne kadar temsil ettiğini gösterir. OOP’de sınıfların belirli bir amaca yönelik yazılması beklenir. (High Cohesion) Örnek verecek olursak matematik işlemi yapan bir sınıfa file işlemlerini gerçekleyen bir metod daha eklediğimiz zaman high Cohesion prensibini bozmuş oluruz.
...
Java Reflection bir uygulama geliştirme arayüzü olup, o anki JVM’deki sınıfları, arayüzleri ve nesneleri yansıtan, temsil eden kütüphanedir. Özellikle geliştirme araçları yazarken reflection kütüphanesine ihtiyaç duyarız. Reflection kütüphanesini kullanarak; -Bir nesnenin sınıfını belirleriz -Classın fieldları, metodları, constructorları, üst classları ve değişkenleri hakkında bilgiler alabiliriz -Bir arayüze ait olan sabit ve metod tanımlamalarını tespit ederiz -Çalışma zamanına kadar ismi bilinmeyen bir sınıf örneği yaratabiliriz (Bir bakıma runtime’da yeni sınıflar yaratabilirsiniz) -Çalışma zamanına kadar ismi bilinmese de sahaların değerini ayarlar ve alabiliriz. -Aynen çalışma zamanına kadar bilinmeyen metodları çalıştırabiliriz. -Çalışma zamanına kadar bileşenleri ve boyutu bilinmeyen diziler yaratabilir, daha sonra bu dizinin bileşenlerini değiştirebiliriz.Küçük projelerde yapılan işlem sayısı, kullanılan tip sayısı pek fazla değildir. Buna bağlı olarak yazılan if-else blokları ve switch-case blokları nispeten küçük olacağından okunmasında, takip edilmesinde çok sıkıntı yaşanmaz. Reflection ve annotation’lar bu durumların tersinin ortaya çıktığı genellikle büyük ölçekli projelerde kullanışlı mekanizmalardır.
...
Mikroservis yekpare bir sistemi her biri bağımsız olarak çalışan e açık protokoller (http, soap) vasıtasıyla birbiri ile iletişim kuran küçük servislere ayrılmasını konu alır. Monolitik yapıya bir alternatif olarak düşünülmüştür. Monolitik mimaride yapı tektir, nispeten anlaşılması kolaydır ve tek bir dille geliştirilebilir. Uygulama büyüdükçe codebasein yönetilmesi, bakımının ve deploy edilmesi zorlaşır. Mikroservisler birbirinden bağımsız ve tek bir işe odaklanmıştır,  Uygulama yatay eksende daha kolay scale edilebilir.
